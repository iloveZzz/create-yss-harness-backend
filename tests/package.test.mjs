...
test('固定包契约、真实模板计划零写入和旧实例拒绝',t=>{
 const scratch=fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(),'create-yss-harness-backend-')));t.after(()=>fs.rmSync(scratch,{recursive:true,force:true}));
 const target=path.join(scratch,'project');
 const run=(...args)=>spawnSync(process.execPath,[path.join(root,'bin/create-yss-harness-backend.js'),...args,'--json'],{encoding:'utf8',maxBuffer:32*1024*1024});
 ...
 fs.mkdirSync(target);const metadata=path.join(target,'.yss-harness-backend.json');fs.writeFileSync(metadata,'{"schema_version":1}');
 r=run('attach','--target-dir',target,'--apply','--force');assert.equal(r.status,1);assert.equal(JSON.parse(r.stdout).code,'LEGACY');assert.deepEqual(fs.readdirSync(target),['.yss-harness-backend.json']);
});
