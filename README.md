...
npx create-yss-harness-backend@latest sync --target-dir ./my-project
npx create-yss-harness-backend@latest sync --target-dir ./my-project --apply
...
每次应用保存 `.yss-harness-state/backend/transactions/<id>/journal.json` 和原文件备份。失败自动恢复；进程中断时预览只诊断，下次 `--apply` 先恢复，再重新执行计划。恢复遇到后续修改会停下并保留恢复清单。成功后也保留备份，首版无独立 rollback 命令。状态目录应由项目自行排除 Git；CLI 不改已有 Git 配置。
